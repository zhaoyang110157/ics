...
bool_t set_long_val(mem_t *m, long_t addr, long_t val)
{
    ...
    if (addr < 0 || addr + 8 > m->len)
	    return FALSE;
//fine, this fucking thing is handled for us.
//Thanks,TA.
//I take it back.
    for (i = 0; i < 8; i++) {
...
//to compare if the two memory is the same 
    long_t pos;
    int len = oldm->len;
    bool_t diff = FALSE;
 ...
 */
cc_t compute_cc(alu_t op, long_t argA, long_t argB)
{
...
bool_t cond_doit(cc_t cc, cond_t cond) 
{
    ...
    //add code beneath
    doit = (cond == C_YES);
    doit = doit || ( GET_ZF(cc) == 1 && ( cond == C_LE || cond == C_E || cond == C_GE )) || ( GET_ZF(cc) == 0 && ( cond == C_NE));    
    doit = doit || ( GET_SF(cc) == 1 && ( cond == C_L || cond == C_LE)) || ( GET_SF(cc) == 0 && GET_ZF(cc) == 0 && ( cond == C_G || cond == C_GE)) ;
    return doit;
}
...
stat_t nexti(y64sim_t *sim)
{
    ...
    if( icode == 2 || icode == 3 || icode == 4 || icode == 5 || icode == 6  || icode == 10 || icode == 11){
        if(!get_byte_val(sim->m , next_pc , &regis)){
    	    err_print("PC = 0x%lx, Invalid register address", sim->pc); 
	    return STAT_ADR;
        }
	regA = GET_REGA(regis);
	if( regA != REG_NONE)
		rA = get_reg_val(sim->r, regA); 
	regB = GET_REGB(regis);
	if( regB != REG_NONE)
		rB = get_reg_val(sim->r, regB);
	next_pc++;
    }
	...
	if(!get_long_val(sim->m , next_pc , &imm)){
	    err_print("PC = 0x%lx,Invalid immediate or dest", sim->pc);
	    return STAT_ADR;
	}
	next_pc += 8;
    }
    ...
    switch (icode) {
      ...
      case I_JMP: /* 7:x imm */
	   ...
	   break;
      case I_CALL: /* 8:x imm */
	    tmp = get_reg_val(sim->r, REG_RSP );
	    set_reg_val(sim->r , REG_RSP, tmp-8);
	    if(! set_long_val(sim->m , tmp-8 , next_pc)){
		err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,tmp-8);
		return STAT_ADR;
		}
	    set_long_val(sim->m , tmp-8 , next_pc);
	    next_pc = imm;
	    break;
      case I_RET: /* 9:0 */
	    tmp = get_reg_val(sim->r, REG_RSP);
	    set_reg_val(sim->r , REG_RSP, tmp+8);
	    if( tmp < 0 || tmp > sim->m->len-8){
		err_print("PC = 0x%lx, Invalid stack address 0x%lx", next_pc,tmp+8);
		return STAT_ADR;
		}
	    get_long_val(sim->m , tmp , &rA);
	    next_pc = rA;
	    break;
      case I_PUSHQ: /* A:0 regA:F */
	    tmp = get_reg_val(sim->r, REG_RSP );
	    set_reg_val(sim->r , REG_RSP , tmp-8);
	    if( tmp <= 8 || tmp > sim->m->len){
		err_print("PC = 0x%lx, Invalid stack address 0x%lx", sim->pc,tmp-8);
		return STAT_ADR;
		}
	    set_long_val(sim->m , tmp-8 , rA);
	    break;
      case I_POPQ: /* B:0 regA:F */
	    ...
	    get_long_val(sim->m , tmp , &rB);
	    set_reg_val(sim->r, regA, rB);
	    break;
      default:
