...
//to extract the hostname, path and port component from an uri
int parse_uri(char *uri, char *target_addr, char *path, char *port);

//to cinstruct an entry for the log file in the proper format
void format_log_entry(char *logstring, struct sockaddr_in *sockaddr,
			 char *uri, size_t size);

//the work for the thread, to connect
void* thread(struct conninfo* vargp);

//create the new conninfo
struct conninfo* new_conn(int fd, struct sockaddr_storage addr);

//proxy handler
int doit(int fd, struct sockaddr_storage* addr);

//request received handler
int parse_req(char* buf, struct reqheaderinfo* reqp);

//interact with server and send response to client
size_t interact(struct reqheaderinfo* reqp, char* reqheadersp,
		int nreqheaders, char* reqbodyp,
		int reqbodylen, int fd_send_resp); 
...
{
    //Init varies
    int listenfd, connfd; //socket for conn and listen 
    socklen_t clientlen;   
...
int doit(int fd, struct sockaddr_storage* sockaddr_browser){
    ...
    format_log_entry(logstring,(struct sockaddr_in*)sockaddr_browser , url ,size);
...
int parse_uri(char *uri, char *hostname, char *pathname, char *port)
{
    ...
    if (strncasecmp(uri, "http://", 7) != 0) {
        hostname[0] = '\0';
        return -1;//hostname has a wrong format
    }

    /* Extract the host name */
    hostbegin = uri + 7;
    hostend = strpbrk(hostbegin, " :/\r\n\0");//find the location of the four char of the string 2
    if (hostend == NULL)
        return -1;//wrong format
    len = hostend - hostbegin;
    strncpy(hostname, hostbegin, len);//from begin to origin
    hostname[len] = '\0';
    ...
    if (*hostend == ':') {
        ...
        while (isdigit(*p))
            *port++ = *p++;
        *port = '\0';
    } else {//without mark port,so set port 80
        strcpy(port, "80");
    }
